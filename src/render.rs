...
/// Bytes per logical frame: 480 * 270 * 4 = 518400 (~506 KiB).
...
impl Framebuffer {
    ...
    /// Raw RGBA8 row-major pixels, ready for GPU upload.
    pub fn pixels(&self) -> &[u8] {
        &self.pixels
    }

    /// Raw mutable RGBA8 row-major pixels.
...
impl Renderer for Framebuffer {
    ...
    fn fill_rect(&mut self, x: i32, y: i32, w: i32, h: i32, color: Color) {
        ...
        for py in y0..y1 {
            let base = (py * stride + x0) * 4;
            let row = &mut self.pixels[base as usize..(base + (x1 - x0) * 4) as usize];
...
mod tests {
    ...
    fn clear_fills_every_pixel() {
        ...
        assert_eq!(pixel(&fb, 0, 0), [1, 2, 3, 255]);
        assert_eq!(
        ...
        assert!(
            fb.pixels()
                .chunks_exact(4)
                .all(|px| { px[0] == 1 && px[1] == 2 && px[2] == 3 && px[3] == 255 })
        );
    }
    ...
    fn fill_rect_clips_to_bounds() {
        ...
        assert_eq!(pixel(&fb, WIDTH as i32 - 1, 0), [0, 0, 0, 255]);
    }
    ...
    fn fill_circle_matches_radius() {
        ...
        assert_eq!(pixel(&fb, 10, 10), [255, 255, 255, 255]);
        // Inside the radius.
        assert_eq!(pixel(&fb, 10, 14), [255, 255, 255, 255]);
        assert_eq!(pixel(&fb, 14, 10), [255, 255, 255, 255]);
        // Outside the radius (5,5 distance 7.07 > 5).
        assert_eq!(pixel(&fb, 5, 5), [0, 0, 0, 255]);
        assert_eq!(pixel(&fb, 16, 10), [0, 0, 0, 255]);
    }
    ...
    fn draw_text_paints_glyphs() {
        ...
        assert_eq!(pixel(&fb, 0, 0), [0, 0, 0, 255]);
        assert_eq!(pixel(&fb, 1, 0), [0, 0, 0, 255]);
        assert_eq!(pixel(&fb, 2, 0), [255, 255, 255, 255]);
        assert_eq!(pixel(&fb, 7, 0), [255, 255, 255, 255]);
        assert_eq!(pixel(&fb, 3, 3), [255, 255, 255, 255]);
        assert_eq!(pixel(&fb, 3, 7), [0, 0, 0, 255]);
    }
}
