//! The main app: game selection, the player-count menu, and the desktop/Android
...
pub fn desktop_main() {
    miniquad::start(conf(), || {
        Box::new(engine::app::Stage::new(Box::new(
            game_select::GameSelect::new(),
        )))
    });
}

/// Android entry point, called from the Java glue (`quad_main`), which in turn
/// is invoked from `MainActivity.onCreate` after `System.loadLibrary("app")`.
